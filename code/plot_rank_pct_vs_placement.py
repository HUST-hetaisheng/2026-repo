"""
Model 2 可视化：两种方法的差异 vs 最终名次
左图：裁判Rank - 粉丝Rank vs 最终名次
右图：裁判Percent - 粉丝Percent vs 最终名次
"""
...
def compute_weekly_metrics(df_fan):
    ...
    for (season, week), group in df_fan.groupby(['season', 'week']):
        ...
        fan_pct = valid['fan_vote_share'].values  # 已经是份额
...
for name, info in CONTROVERSIAL.items():
    ...
    if len(subset) > 0:
        ...
        print(f"  Rank差 (J-F): {row['avg_rank_diff']:+.2f} (归一化: {row['rank_diff_norm']:+.3f})")
        print(f"  平均裁判%: {row['avg_judge_pct']:.1%}, 平均粉丝%: {row['avg_fan_pct']:.1%}")
        print(f"  Pct差 (F-J): {row['avg_pct_diff']:+.1%}")
        print(f"  最终名次: {int(row['placement'])}/{int(row['n_contestants'])} (归一化: {row['placement_norm']:.2f})")
...
ax1.set_xlabel('(Judge Rank − Fan Rank) / (N−1)\n← Fan Favored | Judge Favored →', fontsize=11)
...
plt.tight_layout()
plt.savefig('d:/2026-repo/figures/rank_pct_vs_placement.png', dpi=300, bbox_inches='tight')
plt.show()
...
# ============================================================
print("\n" + "=" * 70)
print("争议选手检测（Rank差 > 0.1 且 名次前40%）")
print("=" * 70)
