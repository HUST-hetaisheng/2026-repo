...
def compute_weekly_metrics(df, season):
    """
    对于每周,计算每位选手的归一化Safety Margin (0 = 淘汰线, 1 = 最安全)
    
    Rank Method: 
      - judge_rank + fan_rank (越小越安全)
      - 归一化: (max_combined - my_combined) / (max_combined - min_combined)
    
    Percentage Method:
      - judge_share + fan_share (越大越安全)  
      - 归一化: (my_combined - min_combined) / (max_combined - min_combined)
    """
    ...
    for week in weeks:
        ...
        # 归一化: 0 = 最危险(最高combined_rank), 1 = 最安全(最低combined_rank)
        ...
        # 归一化: 0 = 最危险(最低combined_share), 1 = 最安全(最高combined_share)
...
def plot_counterfactual_survival():
    ...
    plt.tight_layout()
    plt.savefig('d:/2026-repo/figures/counterfactual_survival.png', dpi=300, bbox_inches='tight')
