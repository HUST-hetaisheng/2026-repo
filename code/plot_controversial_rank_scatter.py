...
for name, info in CONTROVERSIAL.items():
    ...
    if len(subset) > 0:
        ...
        print(f"  裁判最后一名: {int(row['judge_last_count'])}次 ({row['judge_last_pct']:.0%})")
        print(f"  裁判倒数两名: {int(row['judge_bottom2_count'])}次 ({row['judge_bottom2_pct']:.0%})")
        print(f"  粉丝前两名: {int(row['fan_top2_count'])}次")
        print(f"  平均Rank差(J-F): {row['avg_rank_diff']:+.2f}")
        print(f"  最终名次: {int(row['placement'])}/{int(row['n_contestants'])}")
...
plt.tight_layout()
plt.savefig('d:/2026-repo/figures/controversial_analysis.png', dpi=300, bbox_inches='tight')
plt.show()
...
# ============================================================
print("\n" + "=" * 70)
print("自动检测争议选手（裁判倒数2率 > 40% 且 名次在前50%）")
print("=" * 70)
