...
{
\setlength{\parskip}{-1pt} % 段间距设为0
\renewcommand{\baselinestretch}{0.9} % 行距设为0.9倍（可按需调整，如 0.8）
\tableofcontents
}
\newpage
...
\textbf{Output:} Copilot generated the \texttt{statsmodels.MixedLM} code structure. We manually verified ICC calculations and cross-checked results against the theoretical formula $\text{ICC} = \sigma^2_u / (\sigma^2_u + \sigma^2_\epsilon)$.
...
\textbf{Output:} Copilot assisted with vectorized simulation code and matplotlib visualization. All statistical interpretations (e.g., "60.2\% fan-favored by Percentage") were computed and verified manually.
...
\textbf{Output:} Copilot generated the \texttt{tabular} environment structure. Caption text and variable interpretations were written entirely by the team.
