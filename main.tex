...
The raw dataset spans 34 seasons (2005--2024) and contains 421 celebrity--partner pairs with weekly judges' scores and elimination outcomes. We perform several preprocessing steps to prepare the data for inverse modeling.

First, we address missing and placeholder values. In the original data, zeros serve dual purposes: indicating judge absences (rare) and marking post-elimination weeks (common). We distinguish these cases by treating a row as ``active'' only when at least one judge score is positive. For each active contestant-week, we sum all non-zero individual judge scores to obtain the total judges' score $J_{i,t}$. The \texttt{results} column is parsed to extract elimination outcomes; contestants marked as ``Withdrew'' are flagged separately since their exits are not vote-determined and should not impose constraints on fan vote estimation.

Second, we classify each season into one of three regimes based on the show's historical rules \cite{WikiDWTS}: \emph{Rank} (Seasons 1--2), where elimination is determined by summing judges' and fans' ordinal ranks; \emph{Percent} (Seasons 3--27), where normalized score shares are added; and \emph{Bottom2} (Seasons 28--34), where the bottom two contestants by combined rank face a judges' save decision. This regime classification is essential because different rules require different constraint formulations in the inverse problem.

Third, we annotate each week by its elimination pattern---single elimination, double elimination, no elimination (results shows), or finals---since the number and type of constraints vary accordingly. Finals weeks use the \texttt{placement} column to establish the final ranking among survivors.

Finally, we validate data quality by checking that elimination weeks are monotonically increasing (no contestant reappears after elimination), that finals rankings are consistent with the last active set, and that judge scores fall within the expected 1--10 range per judge. After cleaning, the dataset contains 2,699 valid contestant-week observations with complete judge scores and unambiguous elimination status.
...
We use \texttt{results} to identify elimination weeks. 
\texttt{placement} is only used to order contestants \emph{within the same elimination set} 
...
Consistency is evaluated without using placement information, \emph{except}
(i) finals-week ordering and 
(ii) within-week ordering inside a multi-elimination set.
Specifically:
\begin{itemize}
  ...
  \item Finals: ordering matches placement.
\end{itemize}
Uncertainty is reported \emph{by week} and \emph{by contestant} using confidence-interval width
and coefficient of variation, reflecting whether certainty is uniform across weeks or contestants.
