...
% 开篇：问题定义 + 方法预览
\textbf{Research Question}: What pre-competition characteristics of celebrities 
and professional dancers influence their performance on DWTS?

% 数据说明（简洁）
To address this question, we supplemented the original dataset with:
\begin{itemize}
    \item \textbf{Physical attributes}: BMI (Body Mass Index)
    \item \textbf{Prior experience}: Dance experience score (0-10)
    \item \textbf{Social media metrics}: Weekly Twitter, Instagram, Facebook, 
          YouTube engagement (standardized and aggregated)
    \item \textbf{Search interest}: Weekly Google search volume
\end{itemize}

% 方法预览
We employ a \textbf{multi-level regression framework} with three dependent 
variables (judge scores, fan votes, final placement) at two temporal scales 
(contestant-level and weekly-level), complemented by a \textbf{mixed-effects model} 
to isolate the professional dancer effect.
...
score ($j + f$) because: (1) it avoids subjective weighting choices, 
...
\textit{Key distinction}: Static variables (Age, Industry) capture 
\textbf{between-person} differences; dynamic variables (SocialMediaPop) 
capture \textbf{within-season} fluctuations.
...
\textbf{Finding 1: Age Effect.} Younger contestants receive higher judge scores 
($\beta = -0.28$, $p = 0.002$). A 10-year age difference corresponds to 
approximately 2.8 points lower in average judge score.

\textbf{Finding 2: Popularity Drives Fan Votes, Not Judge Scores.} 
Celebrity popularity significantly predicts fan vote share ($p = 0.001$) 
but has no effect on judge scores ($p = 0.46$). This confirms that 
professional evaluation is independent of pre-existing fame.

\textbf{Finding 3: Social Media Stars Outperform.} Contestants from the 
``Social Media Personality'' industry score 5.3 points higher than baseline 
($p = 0.005$), likely due to highly engaged online fanbases.

%------------------------------------------------------------------
\subsection{Professional Dancer Effect}

A unique feature of DWTS is that celebrity contestants are paired with 
professional dancers whose skill, teaching ability, and choreography 
may significantly influence outcomes. We quantify this effect using a 
\textbf{mixed-effects model} with dancers as random effects.
...
\textbf{Conclusion}: 
\begin{itemize}
    \item \textbf{Judges prioritize}: Age (physical capability), 
          dance experience (technical foundation)
    \item \textbf{Fans prioritize}: Pre-existing popularity, 
          real-time social media engagement
    \item \textbf{Both reward}: Industry background (athletes perform well 
          in both evaluations)
\end{itemize}
