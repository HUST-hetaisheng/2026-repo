...
We combine the estimated fan vote shares with the original judge scores and simulate weekly eliminations under two counterfactual rules: the Rank method (rank-sum) and the Percentage method (score-share sum). Table~\ref{tab:rank_pct_metrics} reports both the overall metrics and the regime-level breakdown. The methods agree in 72.2\% of weeks, but still diverge in 93 weeks. Against historical eliminations, the Percentage method shows higher match accuracy (0.695 vs. 0.611). When the two methods disagree, a positive $\Delta_{fan}$ indicates that the Rank method eliminates a contestant with a higher fan share, implying the Percentage method is more fan-friendly in those weeks. Figure~\ref{fig:fan_vs_judge_scatter} visualizes the agreement structure between fan votes and judge scores; deviations from the diagonal highlight weeks where rule choice is most likely to change the elimination outcome.
...
\toprule
\multicolumn{8}{l}{\textbf{Overall (All Seasons)}}\\
\midrule
\textbf{Metric} & \textbf{Value} & \multicolumn{6}{c}{} \\
\midrule
Total weeks analyzed & 335 & \multicolumn{6}{c}{} \\
Method agreement rate & 0.722 & \multicolumn{6}{c}{} \\
Rank accuracy vs actual & 0.611 & \multicolumn{6}{c}{} \\
Percentage accuracy vs actual & 0.695 & \multicolumn{6}{c}{} \\
Disagree weeks & 93 & \multicolumn{6}{c}{} \\
Fan-favored by Rank (count) & 37 & \multicolumn{6}{c}{} \\
Fan-favored by Percentage (count) & 56 & \multicolumn{6}{c}{} \\
Avg $\Delta_{fan}$ (disagree weeks) & 0.0086 & \multicolumn{6}{c}{} \\
Avg eliminated fan share (Rank) & 0.1170 & \multicolumn{6}{c}{} \\
Avg eliminated fan share (Pct) & 0.1146 & \multicolumn{6}{c}{} \\
\midrule
\multicolumn{8}{l}{\textbf{By Regime}}\\
\midrule
\textbf{Regime} & \textbf{Weeks} & \textbf{Agreement} & \textbf{Avg $\Delta_{fan}$ (disagree)} & \textbf{Fan->Rank} & \textbf{Fan->Pct} \\
\midrule
...
\par\vspace{0.3em}
{\footnotesize Note: $\Delta_{fan}>0$ means the Rank method eliminates a contestant with higher fan share, hence the Percentage method is more fan-friendly in those weeks. Higher accuracy indicates closer match to historical eliminations.}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/fan_vs_judge_scatter_side_by_side.png}
\caption{Fan vs. judge agreement structure (left: rank comparison; right: share comparison).}
\label{fig:fan_vs_judge_scatter}
\end{figure}
...
% 开篇：Research Question + 数据/方法综述
\textbf{Research Question}: What pre-competition characteristics of celebrities and professional dancers most significantly influence their performance trajectories and final outcomes on DWTS?

To rigorously address this question, we have augmented the original competition dataset with four complementary dimensions of external data:
\begin{itemize}
    \setlength\itemsep{0em}
    \item \textbf{Physical Attributes:} Body Mass Index for each contestant.
    \item \textbf{Prior Experience:} A quantified Dance Experience Score.
    \item \textbf{Social Media Metrics:} Aggregated weekly engagement across Twitter, Instagram, Facebook, and YouTube (standardized and aggregated).
    \item \textbf{Public Attention:} Weekly Google Search Volume trends.
\end{itemize}
Leveraging this enriched dataset, we construct a comprehensive \textbf{multi-level regression framework}. Our approach analyzes performance at two distinct temporal scales---season-level outcomes and weekly fluctuations---across three key dependent variables: judges' scores, fan votes, and final placement. Furthermore, we employ a \textbf{mixed-effects model} to isolate and quantify the specific impact of professional dancer partners, separating their influence from the celebrity's own merit.
...
\midrule
$Y_1$ & \texttt{avg\_judge\_score} & Professional evaluation (0--40 scale) \\
$Y_2$ & \texttt{avg\_fan\_vote\_share} & Audience preference (0--1 scale) \\
$Y_3$ & \texttt{placement} & Overall success (1 = winner) \\
\bottomrule
\end{tabular}
\caption{Three dimensions of ``success'' on DWTS}
\end{table}

\textit{Note}: We use final \texttt{placement} rather than a weighted composite 
score because of the following reasons: (1) it avoids subjective weighting choices, 
(2) different scoring regimes across seasons make composite scores 
non-comparable, and (3) placement directly answers ``who wins.''

% 3.1.2 模型表达式
\subsubsection{Contestant-Level Model (Static Characteristics)}
To systematically disentangle the drivers of success, we adopt a stratified modeling approach. The first stratum focuses on static characteristics---attributes such as age, industry background, and prior dance experience that remain constant throughout a season. Since these factors define a contestant's baseline potential, we aggregate weekly performance metrics into season-level averages (or final placement) to filter out week-to-week noise.

Furthermore, we construct three parallel regression models for different dependent variables ($Y_1, Y_2, Y_3$). This triangulation allows us to isolate specific mechanisms, which is crucial for identifying the distinct criteria used by the two decision-making bodies.

For contestant $i$:
\begin{equation}
Y_i = \beta_0 + \beta_1 \cdot \text{Age}_i + \beta_2 \cdot \text{isUS}_i 
    + \beta_3 \cdot \text{Popularity}_i + \beta_4 \cdot \text{BMI}_i 
    + \beta_5 \cdot \text{DanceExp}_i + \sum_{k} \gamma_k \cdot \mathbf{1}[\text{Industry}_i = k] 
    + \epsilon_i
\end{equation}

\textit{Sample}: $N = 421$ contestants across 34 seasons.
...
\textit{Sample}: $N = 2{,}777$ contestant-weeks.
...
\midrule
Judge Score (Contestant) & 0.237 & 0.211 & 9.01*** \\
Fan Vote (Contestant) & 0.177 & 0.148 & 6.22*** \\
Placement (Contestant) & 0.241 & 0.215 & 9.21*** \\
\midrule
Judge Score (Weekly) & 0.331 & 0.327 & 97.6*** \\
Fan Vote (Weekly) & 0.319 & 0.316 & 92.6*** \\
\bottomrule
\end{tabular}
\caption{Model fit statistics. ***: $p < 0.001$}
\end{table}

\textit{Interpretation}: The contestant-level $R^2$ of 0.18--0.24 is consistent 
with behavioral research norms, indicating that pre-competition characteristics 
explain a meaningful but limited portion of variance---leaving room for actual 
...
\begin{tabular}{lcccc}
\toprule
Variable & Judge ($\beta$) & Fan ($\beta$) & Placement ($\beta$) & Insight \\
\midrule
Age & $-0.28$** & $-0.002$* & $+0.22$*** & Younger $\to$ higher scores \\
Celeb Popularity & $-0.05$ & $+0.002$*** & $-0.14$** & Popularity $\to$ fan votes \\
Dance Experience & $-0.16$ & $-0.000$ & $+0.17$* & Weak effect \\
Industry: Athlete & $+0.52$ & $+0.004$ & $-0.15$ & Athletes perform well \\
Industry: Social Media & $+5.30$** & $-0.020$ & $+0.31$ & SM stars excel \\
\bottomrule
\end{tabular}
\caption{Selected regression coefficients. *: $p<0.05$, **: $p<0.01$, ***: $p<0.001$}
\label{tab:coefficients}
\end{table}

\textbf{The Age Advantage in Professional Assessment.} 
There is a robust negative correlation between age and professional evaluation. The coefficient ($\beta = -0.28, p = 0.002$) suggests that for every additional decade of age, a contestant's average judge score decreases by approximately 2.8 points. This reflects the physical demands of ballroom dancing, where agility and stamina—traits often correlated with youth—are prerequisites for high technical marks.

\textbf{The Divergence of Fame and Skill.} 
A critical insight is the independence of professional judgment from celebrity status. Pre-existing popularity is a highly significant predictor of fan vote share ($p = 0.001$) but has no statistically discernible effect on judge scores ($p = 0.46$). This confirms that while fan voting acts as a popularity contest, the judges effectively maintain a "blindness" to fame, focusing solely on performance quality.

\textbf{The "Social Media" Industry Effect.}
Contestants categorized as "Social Media Personalities" demonstrate a massive outlier effect in judge scores ($\beta = +5.30, p = 0.005$). This likely stems from a selection bias: social media stars cast on the show are often younger and possess dance-adjacent skills (e.g., TikTok choreography trends), giving them a baseline technical advantage over traditional actors or musicians.
...
\midrule
Age & $-3.06$** & $-2.27$* & \textbf{Judges} \\
Celebrity Popularity & $-0.74$ & $+3.29$*** & \textbf{Fans} \\
Dance Experience & $-1.17$ & $-0.02$ & Judges (weak) \\
BMI & $+0.38$ & $-0.16$ & Similar \\
is\_US & $-0.44$ & $-0.85$ & Similar \\
\bottomrule
\end{tabular}
\caption{Comparison of $t$-statistics across Judge and Fan models}
\end{table}

Our comparative analysis reveals a clear dichotomy in decision criteria. \textbf{Judges} prioritize physical capability and technical foundation, as evidenced by the significant negative impact of age and the relevance of prior dance experience on their scoring. In contrast, \textbf{Fans} are primarily driven by engagement and fame; pre-existing celebrity popularity and real-time social media buzz are strong predictors of vote share but have negligible effect on professional scores. Interestingly, \textbf{Both} groups consistently reward specific industry backgrounds, particularly athletes, suggesting that physical discipline and competitiveness translate well into both technical merit and audience appeal.


This divergence justifies the show's hybrid scoring system: judges ensure 
technical quality is rewarded, while fan votes maintain audience engagement 
and emotional investment.

%------------------------------------------------------------------
\subsection{Professional Dancer Effect}

Celebrity contestants are paired with professional dancers whose skill, teaching ability, and choreography may significantly influence outcomes. 

We quantify this effect using a \textbf{Linear Mixed-Effects Model (LMM)} rather than including dancers as fixed covariates in the previous regression. This separate modeling approach is necessary for two reasons. There are over 50 unique professional dancers across 34 seasons. Including them as separate dummy variables in the OLS model would consume excessive degrees of freedom, making the estimates for static characteristics unstable. Also, by modeling dancers as \textit{random effects}, we can estimate the global variance ($\sigma^2_u$) attributable to the "partner factor" itself, rather than testing individual coefficients. This allows us to calculate how much of the total success is driven by the partner assignment via the Intraclass Correlation Coefficient (ICC).
...
The ICC represents the proportion of outcome variance attributable to 
the professional dancer assignment.
...
\midrule
Judge Score & \textbf{13.7\%} & Dancer explains $\sim$14\% of judge score variance \\
Fan Vote Share & \textbf{15.4\%} & Dancer matters slightly more for audience votes \\
Placement & \textbf{18.2\%} & Dancer assignment most strongly affects final ranking \\
\bottomrule
\end{tabular}
...
provides a substantial advantage. The ICC of 18.2\% for placement means 
that nearly one-fifth of the variation in final ranking can be attributed 
...
(lower is better). Table~\ref{tab:top_dancers} presents the most successful 
professional dancers in DWTS history.
...
\begin{itemize}
    \item \textbf{Derek Hough dominates}: With an average partner placement of 2.94 
          across 17 partners, Derek Hough is statistically the most successful 
          professional dancer. His partners finish in the top 3 on average.
    
    \item \textbf{The Hough siblings}: Julianne Hough (Derek's sister) ranks second 
          despite fewer appearances, suggesting a family advantage in training 
          methodology or partner chemistry.
    
    \item \textbf{Experience matters}: Top-10 dancers average 14.5 seasons 
          on the show, compared to 6.8 for all dancers, suggesting that 
          accumulated experience improves teaching and choreography skills.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Age matters}: Younger contestants have a measurable advantage 
          in judge scores ($\beta = -0.28$, $p = 0.002$), but the effect on fan votes is weaker.
    
    \item \textbf{Popularity $\neq$ Skill}: Celebrity popularity predicts fan votes 
          ($p = 0.001$) but not professional evaluation---judges are ``blind'' to fame.
    
    \item \textbf{Dancer assignment is consequential}: Professional dancers explain 
          14--18\% of outcome variance (ICC). Derek Hough's partners average 
          placement 2.94, making him the statistically most successful professional dancer.
    
    \item \textbf{Dynamic effects exist}: Weekly social media popularity 
          significantly predicts that week's fan vote share ($p < 0.001$), 
          suggesting real-time engagement drives audience voting behavior.
    
    \item \textbf{Industry background matters}: Social media personalities 
          score 5.3 points higher than baseline ($p = 0.005$), likely due to 
          highly engaged online fanbases.
\end{enumerate}
