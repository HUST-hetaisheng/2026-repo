...
\textbf{Task 4: Alternative Scoring System.}
An alternative scoring system combining judges’ scores and fan votes is proposed and evaluated to determine whether it improves fairness or other aspects of the competition.
...
\begin{itemize}
\item \textbf{Seasons 1--2 (Rank)}: Combined rank = judge rank + fan rank; the worst sum is eliminated.
\item \textbf{Seasons 3--27 (Percent)}: Combined score = judge share + fan share; the smallest sum is eliminated.
\item \textbf{Seasons 28--34 (Bottom2 + Judges' Save)}: Bottom two by combined rank; judges choose whom to eliminate.
\end{itemize}
...
We model the judges' decision probabilistically. Given bottom-two contestants $\{e, b\}$, the probability
...
\begin{enumerate}
\item Check feasibility: can fan ranks be assigned so that $\{e, b\}$ form the bottom two?
\item If feasible, compute $\Pr(\text{elim } e \mid e, b)$ via Eq.~\eqref{eq:judges_save}.
\item Select the partner with highest elimination probability (deterministic mode) or sample from
...
\bibitem{1} Boyd, S., \& Vandenberghe, L. (2004). Convex optimization. Cambridge university press.
\bibitem{2} Wikipedia contributors. (2024). Dancing with the Stars (American TV series). Wikipedia.
\bibitem{3} Jaynes, E. T. (1957). Information theory and statistical mechanics. Physical review, 106(4), 620.
\end{thebibliography}
