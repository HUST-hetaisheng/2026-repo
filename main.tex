...
$\star$ \textbf{Violation Identification:} If the observed eliminated contestant $e$ is not within the \text{Bottom}_2 of the combined ranks $\mathbf{R}_w$, the algorithm identifies "offenders"—contestants who are currently in the bottom two but should legally be "safer" than $e$.
...
\subsection{Uncertainty Quantification via Ensemble Simulation}
To satisfy the problem's requirement for "measures of certainty," we do not rely on a single point estimate. For all regimes, we run an \textbf{Ensemble of $N=1,000$ Simulations} per week with perturbed inputs $J_{input} \sim \mathcal{N}(J_{obs}, \sigma_{noise})$. The resulting distribution of fan votes provides both the \textbf{Point Estimate} (the mean of the ensemble $\mu_f$) and the \textbf{Certainty Metric} (the Coefficient of Variation $CV = \sigma_f / \mu_f$), where a high CV indicates that the elimination result could have been caused by many different voting scenarios.
