...
\title{Balancing Technical Merit and Public Appeal: Evaluation and Optimization of the DWTS Scoring Mechanism}
...
% \textbf{For Task 2}, we conducted \textbf{counterfactual simulations} to compare different score combination rules: \textbf{the Rank‑Sum method} and \textbf{the Percentage‑Sum method}. The simulations are totally different in \textbf{93 weeks} across all seasons.The Percentage method protects the contestant with higher fan support in \textbf{60.2\% }of instances, acting as a \textbf{“fan‑power amplifier”} while the two‑stage elimination framework \textbf{(Judges’ Choice)} significantly reduces the survival probability of low‑skill but high‑popularity contestants, serving as an effective correction mechanism.

% \textbf{For Task 3}, to assess the influence of celebrity characteristics and dancing partners, we employed a \textbf{multi‑level regression framework} and a \textbf{Linear Mixed‑Effects Model} to quantify the effects of judges and parterns. Static‑trait analysis shows that judges’ scores are negatively correlated with contestant's \textbf{age}, and \textbf{social media celebrities} are the most likely to stand out. Dynamic weekly analysis confirms \textbf{social‑media popularity} predicts fan vote share. The Mixed-Effects model reveals that good \textbf{dancing partners} do boost the final results with better judge scores. 

% \textbf{For Task 4} we frame a Weekly Dynamic Weighting System as an optimal-control problem, optimizing the judge-score weight \(\lambda_t\) to balance \textbf{Commercial Value} , \textbf{Fairness} , \textbf{Fan Satisfaction} , and \textbf{Celebrity Benefit} . \textbf{Differential-Evolution-based dynamic control} outperforms static strategies, achieving a \textbf{43.5\%} benefit improvement in Season 25. The optimal \textbf{``N-Shape'' trajectory} phases judge influence to accumulate traffic, enforce a mid-season technical screen, boost engagement, and ensure a qualified winner, raising the show's overall benefit by \textbf{22.9\%}.

% \textbf{Finally}, we offer practical recommendations for the producers of \emph{Dancing with the Stars} regarding fair voting rule optimization. Our ``N-shape'' strategy can be extended to other entertainment contests as well.
% \begin{keywords}
...
\begin{abstract}
\par In \textit{Dancing with the Stars}, judges' scores and fan votes jointly determine outcomes, with different scoring rules directly affecting fairness. We establish a systematic framework to estimate confidential fan votes, evaluate voting rules, quantify contestant and partner effects, and propose improved scoring mechanisms.

\textbf{For Task 1}, we develop an \textbf{entropy-inertia regularized inverse optimization model} to estimate latent weekly fan vote shares across three historical scoring regimes. By maximizing entropy and enforcing temporal smoothness, the model reconstructs elimination outcomes with \textbf{93\%} consistency. Uncertainty is quantified via ensemble inference, revealing regime-dependent confidence patterns.

\textbf{For Task 2}, \textbf{counterfactual simulations} compare the Rank-Sum and Percentage-Sum methods, which diverge in \textbf{93 weeks}. The Percentage method protects higher fan-support contestants in \textbf{60.2\%} of cases, acting as a ``fan-power amplifier,'' while the Judges' Choice framework reduces survival probability for low-skill, high-popularity contestants.

\textbf{For Task 3}, a \textbf{multi-level regression framework} and \textbf{Linear Mixed-Effects Model} quantify celebrity and partner effects. Judges' scores correlate negatively with \textbf{age}; \textbf{social media celebrities} excel. Weekly \textbf{social-media popularity} predicts fan votes, and top \textbf{dancing partners} significantly boost final placements.

\textbf{For Task 4}, we optimize a Weekly Dynamic Weighting System balancing \textbf{Commercial Value}, \textbf{Fairness}, \textbf{Fan Satisfaction}, and \textbf{Celebrity Benefit}. \textbf{Differential-Evolution-based control} achieves \textbf{43.5\%} benefit improvement in Season 25. The optimal \textbf{``N-Shape'' trajectory} phases judge influence to accumulate traffic, enforce mid-season screening, and ensure qualified winners, raising overall benefit by \textbf{22.9\%}.

\textbf{Finally}, we offer practical recommendations for fair voting optimization, with the ``N-shape'' strategy extensible to other entertainment contests.
...
{
\setlength{\parskip}{-2.7pt} % 段间距
\renewcommand{\baselinestretch}{0.86} % 行距设为0.9倍（可按需调整，如 0.8）
\tableofcontents
}
\newpage
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{figures/Our Work.png}
    \caption{Our Work}
    \label{fig:placeholder}
\end{figure}
...
\textbf{Assumption 1 (Time Smoothness):}
For contestants still competing in the competition, their fan vote share changes smoothly between consecutive weeks.

\textbf{Assumption 2 (Maximum Entropy):}
In the absence of restrictive information, assume fan votes are distributed as uniformly as possible.

\textbf{Assumption 3 (Judge Selection Bias):}
When choosing between the bottom two contestants, judges prefer to save the one with the higher score, though this decision is not deterministic.

\textbf{Assumption 4 (Independence):}
Judge scores are assumed independent across contestants and weeks, and viewer votes are considered independent from judge scores.

\textbf{Assumption 5 (Temporal Consistency):}
Contestant votes and judge scores are based solely on the contestant's performance during the current week, with no time lag effects.

\textbf{Assumption 6 (Absence of Extreme Events):}
Viewer votes within a season are unaffected by major stage accidents or social media backlash, focusing solely on the contestant and their performance itself.
...
\begin{equation}
    f_{i} = \frac{e^{\gamma \cdot (r^F_{i} - 1)}}{\sum_k e^{\gamma \cdot (r^F_{k} - 1)}}
\end{equation}
...
\begin{equation}
    P(\text{Elim } a \mid a, b \in \mathcal{B}) = \frac{1}{1 + e^{\beta(J_a - J_b)}}
...
\begin{equation}
    f_{i,w} = \frac{e^{\gamma (r^{F*}_{i,w} - 1)}}{\sum_{k=1}^N e^{\gamma (r^{F*}_{k,w} - 1)}}
\end{equation}
...
In conclusion, our model demonstrates strong empirical validity: it correctly recovers elimination outcomes with high probability (overall $\bar{P}_{\text{cons}} = 0.93$), while uncertainty quantification reveals interpretable regime-dependent patterns that align with the theoretical information content of each voting mechanism\cite{r6}. The Percent regime provides the most reliable inference, while the judges' Choice mechanism in recent seasons introduces irreducible ambiguity that is faithfully captured by our probabilistic framework.
\section{Rank VS Percent: Choice of Score Combination and Elimination Rules} 
\subsection{Influence of Voting Methods on Final Outcomes}

Estimated fan vote shares are combined with original judges’ scores to simulate eliminations under two counterfactual rules: the Rank method (rank-sum) and the Percentage method (score-share sum). As shown in Table~\ref{tab:rank_pct_overall} and ~\ref{tab:rank_pct_regime}, the two methods agree in 72.2\% of weeks but differ in 93 cases. In weeks of disagreement, a positive $\Delta_{fan}$ indicates that the Rank method eliminates a contestant with higher fan support, implying that the Percentage method is more fan-favorable.
...
% \caption{Rank vs. Percentage simulation metrics (overall).}
% \rowcolors{2}{gray!10}{white}
% \label{tab:rank_pct_overall}
% \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l r @{}}
% \toprule
% \multicolumn{2}{l}{\textbf{Overall (All Seasons)}}\\
% \midrule
% Total weeks analyzed & 335 \\
% Method agreement rate & 0.722 \\
% Disagree weeks & 93 \\
% Fan-favored by Rank (count) & 37 \\
% Fan-favored by Percentage (count) & 56 \\
% Avg $\Delta_{fan}$ (disagree weeks) & 0.0086 \\
% Avg eliminated fan share (Rank) & 0.1170 \\
% Avg eliminated fan share (Pct) & 0.1146 \\
% \bottomrule
% \end{tabular*}

% \par\vspace{0.5em}
% {\footnotesize 
% \noindent \textbf{Note:} $\Delta_{fan}>0$ implies the Rank method eliminates a contestant with a higher fan share compared to the Percentage method. Fan-favored counts indicate the number of weeks in which the corresponding method aligns more closely with the popular vote.}
% \end{table}
...
\begin{tabular}{p{0.78\textwidth} >{\raggedleft\arraybackslash}p{0.22\textwidth}}
\toprule
% \rowcolor{gray!15}
\multicolumn{2}{l}{\textbf{Overall (All Seasons)}} \\
\midrule
...
{\footnotesize
\noindent \textbf{Note:}
$\Delta_{fan}>0$ implies the Rank method eliminates a contestant with a higher fan share compared to the Percentage method. Fan-favored counts indicate the number of weeks in which the corresponding method aligns more closely with the popular vote.
}
\end{table}
...
\setlength{\tabcolsep}{0pt}
\renewcommand{\arraystretch}{1.25}
\caption{Rank vs. Percentage simulation metrics by voting regime.}
\label{tab:rank_pct_regime}
...
\toprule
% \rowcolor{gray!15}
\textbf{Regime} & \textbf{Weeks} & \textbf{Agreement} &
\textbf{Avg $\Delta_{fan}$} & \textbf{Fan$\to$Rank} & \textbf{Fan$\to$Pct} \\
\midrule
...
\begin{equation}
    P(\text{Save } i \mid i, j \in \mathcal{B}) = \frac{e^{\gamma(J_i - J_j)}}{1 + e^{\gamma(J_i - J_j)}}
\end{equation}
...
Our simulation results highlight eight participants with significantly "different" performance trajectories under the two different rules. As shown in Figure~\ref{fig:survival_curves}, the percentage rule acted as a significant "popularity amplifier." For participants like Bobby Bones, Iman Shumpert, and Nelly, the percentage rule allowed them to effectively compensate for their technical shortcomings with their immense popularity, significantly increasing their cumulative survival probability. As shown in Figure~\ref{fig:survival_curves1}, the percentage rule effectively protected controversial contestants (those with differing opinions from judges and the audience), pulling them back from the "elimination zone." Conversely, under the ranking point rule, performers like Jerry Rice, Bristol Palin, and Billy Ray Cyrus found their technical disadvantages more difficult to overcome due to the upper limit on ranking points.
\begin{figure}[H]
    \centering
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{figures/method_comparison_analysis.png}
    \caption{Comparative Analysis of Elimination Mechanisms: Rank Aggregation vs. Proportional Voting}
    \label{fig:survival_curves1}
\end{figure}
\begin{table}[htbp]
\centering
...
\begin{tabular}{lcccc}
\hline
\rowcolor{grey!10} Contestant (Season) & Actual Rank & Rank Rule Est. & Percent Rule Est. & Survival Advantage \\
\midrule
...
\begin{tabular}{lccc}
\toprule
\rowcolor{grey!10}Model & $R^2$ & Adj. $R^2$ & $F$-statistic \\
\midrule
...
\toprule
\rowcolor{grey!10}Variable & Judge $\beta$ & $p$ & Fan $\beta$ & $p$ & Place $\beta$ & $p$  \\
\midrule
...
The following table shows the difference between factors about how they influence the judges and the fans.
...
\begin{tabular}{lccc}
\toprule
\rowcolor{grey!10}Factor & Judge ($t$-stat) & Fan ($t$-stat) & Who cares more? \\
\midrule
...
\begin{tabular}{lcl}
\toprule
\rowcolor{grey!10}Dependent Variable & ICC & Interpretation \\
\midrule
...
% \subsubsection{Partner Random Effects}

The mixed-effects model estimates a random effect $u_j$ for each of the 60 professional dancers, representing their ``added value'' beyond contestant characteristics. Figure~\ref{fig:partner_random_effects} displays the top 10 and bottom 10 dancers ranked by their random effects across all three outcome variables.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/task3_me_top_bottom_partners.png}
    \caption{Random effects ($u_j$) for professional dancers across three outcome models. Green bars indicate positive contributions---higher scores or better placements; red bars indicate negative contributions. The horizontal dashed line separates the top 10 from the bottom 10 dancers.}
    \label{fig:partner_random_effects}
\end{figure}

% Several insights emerge from the random effect estimates:

\begin{itemize}
    \item \textbf{Derek Hough dominates all metrics.} He exhibits the highest positive effect on judge scores ($u_j = +2.78$) and the strongest negative effect on placement ($u_j = -1.37$, indicating his partners finish approximately 1.4 positions higher than expected). This confirms his status as the most impactful professional dancer in DWTS history.
    
    \item \textbf{Judge score effects are more dispersed.} The standard deviation of $u_j$ for judge scores is larger than for fan votes, suggesting that partner quality has a more pronounced effect on technical evaluation than on audience popularity.
    
    \item \textbf{Some dancers underperform expectations.} Partners such as Louis van Amstel and Gleb Savchenko show consistently negative random effects, indicating their celebrity partners tend to underperform relative to their baseline characteristics.
\end{itemize}

\subsubsection{Top Professional Dancers}

We rank dancers by the average placement of their partners 
(lower is better). Figure~\ref{fig:partner_ranking} presents the most successful professional dancers in DWTS.
...
% \end{tabular}
% \end{table}
\textbf{Derek Hough }clearly dominates in terms of professional success: across 17 partners, his average partner placement is 2.94. However, we also found that while the first-place partner did indeed have the highest average judges' score, these scores did not necessarily indicate the partner's overall average score. Julianne Hough, who ranked second, only received an average judges' score of 23.6. Meanwhile, Valentin Chmerkovskiy, who had the same average score as Derek at 28.7, only ranked fifth among the partners, with an average placement of 5.26. This suggests that \textbf{audience preferences} are \textbf{less} significantly influenced by the dance partners.
...
\section{Optimization of Competition Mechanism: \\Static Equilibrium and Dynamic Control}
...
We define the global objective function $J$ as a multi-objective optimization problem. To quantify the commercial dimension, we introduce the \textbf{Commercial Value Index (CCVI)}, explicitly constructed based on the \textbf{PID (Proportional-Integral-Derivative)} framework:
\noindent \textbf{Mathematical definition of PID components in CCVI:}
Program commercial impact is modeled as a dynamic signal response, where the optimal parameters $\vec{\lambda}$ must balance three commercial drivers:
\begin{itemize}
    \item \textbf{Proportional Gain ($P$) represents:} Program \textbf{peak search volume}. $P$ determines the program's ability to command premium pricing during high-value advertising slots.
    \item \textbf{Integral Gain ($I$):} Represents \textbf{Cumulative Traffic Volume} ($I = \int \text{Traffic} dt$). $I$ measures audience stickiness and long-term brand loyalty, critical for streaming subscription retention.
    \item \textbf{Derivative Gain ($D$):} Represents \textbf{viral spread rate}. $D$ captures the speed of “heat” propagation and social media ROI, reflecting marketing investment efficiency.
\end{itemize}

To maintain fairness in the competition, we maximize -$f_{Fair}$ (the discrepancy between fan and judge vote shares) and employ a dynamic rescaling mechanism. This recalibrates the vote shares of surviving contestants after eliminations to sum to 1, thereby preventing “denominator drift” in counterfactual simulations. Furthermore, the $f_{Satis}$ term represents audience satisfaction, measured by the correlation between audience vote rankings and final actual rankings. The $f_{Celeb}$ term reflects celebrity popularity and attention, where its growth enhances the celebrity's personal value and social influence.
Combining the above elements, we derive the final optimization objective function:
\begin{equation}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lcccc}
        \toprule
       \rowcolor{grey!10} \textbf{Season} & \textbf{Stage I (Baseline)} & \textbf{Stage II (Fixed Opt)} & \textbf{Stage III (Dynamic Opt)} & \textbf{Total Gain} \\
        \midrule
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{figures/task4_final_comparison.png}
    \caption{Comparison of Three Strategies}
    \label{fig:comprehensive_plots}
\end{figure}
Figure \ref{fig:comprehensive_plots} visualizes the results. Subplot (A) highlights the benefit gap between static and dynamic strategies. Subplot (B) reveals the underlying logic of the optimized dynamic weight for Season 25, showing a distinct "\textbf{N-Shape}" curve.
...
In summary, Task 4 proves that commercial peak and competitive fairness can be harmonized through temporal separation of power, provided the weighting system is dynamically calibrated to the specific signal of each season.Furthermore, the event organizers can also influence the direction of the competition by adjusting the weights of the various components of $J(\vec{\lambda})$ , thereby increasing their control over the event.

% \newpage
\section{Model Analysis}
\subsection{Strength}
\noindent The innovation and advantages of this study lie in constructing a model for estimating audience votes and optimizing scoring rules in \emph{Dancing with the Stars}. Its main advantages can be summarized in the following five points:
\begin{itemize}
\item[ \ding{52} ]\textbf{Entropy–inertia inverse optimization} reconstructs unobserved audience votes solely from elimination outcomes and judges' scores, ensuring objectivity via entropy maximization and behavioral realism through temporal smoothing.
\item[ \ding{52} ]\textbf{Regime-aware modeling} flexibly adapts to three historical scoring rules (rank, percentage, judges’ choice), enabling cross-season consistency and counterfactual rule simulations.
\item[ \ding{52} ]\textbf{Dynamic weight control} optimizes judge–audience weighting over time, allowing phased strategies (e.g., “N-shaped”) to balance engagement, fairness, and brand value.
\item[ \ding{52} ]\textbf{External dynamic predictors} (e.g., social media and search trends) enhance vote modeling, revealing audience votes track real-time popularity while judges’ scores align with technical factors like contestant age.
\item[ \ding{52} ]\textbf{Ensemble inference with uncertainty quantification} perturbs judges’ scores to generate vote-share confidence intervals, increasing transparency under judges’ choice and other rule regimes.
\end{itemize}
\subsection{Weakness and Further Discussion}
\begin{itemize}
\item [ \ding{56} ] Historical rule transition points are inferred rather than officially documented. This assumption-based periodization may introduce subtle inconsistencies when reconstructing past competition dynamics.    
\item [ \ding{56} ]The dynamic control system employs a deterministic optimization framework. It does not account for stochastic viewer behavior or external shocks, which may limit robustness in volatile scenarios.    
\item [ \ding{56} ]The ensemble method perturbs judges' scores under an independence assumption. This may not capture structured scoring biases, leading to an incomplete characterization of estimation uncertainty.
\end{itemize}
\subsection{Conclusion}
\noindent This study develops a computational framework to analyze voting and scoring in \emph{Dancing with the Stars}. It introduces an inverse optimization model to estimate latent audience votes and adapts this analysis across different historical scoring regimes. A dynamic control system is designed to balance viewer engagement with competitive fairness over time. The framework integrates external data and quantifies uncertainty, providing a principled tool for evaluating and designing engaging competition formats.
...
\section{Inverse Optimization for Fan Vote Estimation}

\begin{lstlisting}[language=Python, caption={Fan Vote Estimation via Entropy-Regularized Inverse Optimization}]
def estimate_fan_votes(week_data, lambda_ent=0.1, rho_smooth=0.2):
    """Estimate latent fan vote shares using MaxEnt + elimination constraints."""
    n = len(week_data)
    
    def objective(f):
        entropy = -np.sum(f * np.log(f + 1e-12))
        kl_div = np.sum(f * np.log(f / prior + 1e-12))
        return -lambda_ent * entropy + kl_div + rho_smooth * np.sum((f - f_prev)**2)
    
    constraints = [{'type': 'eq', 'fun': lambda f: np.sum(f) - 1.0},
                   {'type': 'ineq', 'fun': lambda f: elimination_constraint(f, week_data)}]
    return minimize(objective, x0=np.ones(n)/n, method='SLSQP',
                    bounds=[(0.01, 0.99)]*n, constraints=constraints).x
\end{lstlisting}

\section{Mixed-Effects Model for Partner Effect}

\begin{lstlisting}[language=Python, caption={Mixed-Effects Model with Partner Random Effects}]
def fit_partner_effect_model(df, outcome='placement'):
    """Fit LMM: Y_ij = X_ij * beta + u_j + epsilon_ij"""
    X = sm.add_constant(pd.get_dummies(df['Industry'], drop_first=True)
                        .join(df[['age', 'bmi', 'dance_experience_score']]))
    model = MixedLM(df[outcome], X, groups=df['Partner'])
    result = model.fit(method='powell')
    
    var_u, var_e = result.cov_re.iloc[0,0], result.scale
    return {'icc': var_u/(var_u+var_e), 'random_effects': result.random_effects}
\end{lstlisting}

\section{Counterfactual Elimination Simulation}

\begin{lstlisting}[language=Python, caption={Rank vs. Percentage Rule Comparison}]
def simulate_elimination(week_data, method='percentage'):
    """Compare elimination outcomes under different voting rules."""
    j, f = week_data['judge_score_share'], week_data['fan_vote_share']
    if method == 'percentage':
        combined = j + f
    else:  # rank-based
        combined = -(j.rank(ascending=False) + f.rank(ascending=False))
    return week_data.loc[combined.idxmin(), 'contestant']
\end{lstlisting}
...
\textbf{Query:} Design an entropy-regularized inverse optimization model to estimate latent fan vote shares from elimination outcomes.\\
\textbf{Human Input:} We identified that naive regression approaches introduce confirmation bias. We required an unbiased approach using only elimination constraints.\\
\textbf{Output:} Copilot assisted in formulating the MaxEnt + KL-divergence objective function and implementing the SLSQP solver in Python. All hyperparameter choices ($\lambda$, $\rho$) were manually tuned via cross-validation.

\bibitem{AI2}
GitHub Copilot (Ask Mode)\\
\textbf{Query:} Implement a Mixed-Effects Model to quantify professional dancer impact on contestant placement.\\
\textbf{Human Input:} We recognized that including dancer identity as fixed effects would consume excessive degrees of freedom. We decided to treat dancers as random effects to estimate the global variance attributable to partner assignment.\\
\textbf{Output:} Copilot generated the \texttt{statsmodels.MixedLM} code structure. We verified ICC calculations and cross-checked results against the theoretical formula $\text{ICC} = \sigma^2_u / (\sigma^2_u + \sigma^2_\epsilon)$.

\bibitem{AI3}
GitHub Copilot (Ask Mode)\\
\textbf{Query:} Generate counterfactual survival curves comparing Rank-Sum and Percentage-Sum elimination rules.\\
\textbf{Human Input:} We specified the logistic survival function with threshold $\alpha=2.2$ and sensitivity $\beta_s=1.5$ based on empirical analysis of historical elimination patterns. The "Bobby Bones anomaly" was identified as a key test case.\\
\textbf{Output:} Copilot assisted with vectorized simulation code and matplotlib visualization. All statistical interpretations were computed and verified manually.

\bibitem{AI4}
GitHub Copilot (Ask Mode)\\
\textbf{Query:} Debug web scraping pipeline for social media follower counts and Google Trends data.\\
\textbf{Human Input:} We designed the data collection strategy targeting Twitter, Instagram, Facebook, and YouTube. Rate limiting and API authentication were handled manually.\\
\textbf{Output:} Copilot suggested error handling patterns and pandas data cleaning operations. All scraped data was manually validated against public profiles before integration.

\bibitem{AI5}
GitHub Copilot (Ask Mode)\\
\textbf{Query:} Format regression coefficient tables and ICC results in LaTeX with proper significance notation.\\
\textbf{Human Input:} We specified the three-level significance scheme and requested a compact table layout suitable for two-column format.\\
\textbf{Output:} Copilot generated the \texttt{tabular} environment structure. Caption text and variable interpretations were written by the team.
