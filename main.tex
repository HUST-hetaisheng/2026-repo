...
\subsubsection{Summary of Assumption Validity}

The assumptions above are justified by:
\begin{itemize}
    \item \textbf{Empirical evidence:} Rule documentation \cite{WikiDWTS}, Google Trends correlation studies.
    \item \textbf{Mathematical necessity:} Convexity, simplex constraints, maximum entropy are standard in inverse problems \cite{Jaynes1957, Boyd2004}.
    \item \textbf{Model diagnostics:} High CSR (Table~\ref{tab:csr}) and bounded uncertainty (Table~\ref{tab:certainty}) confirm that assumptions are not contradicted by data.
\end{itemize}
