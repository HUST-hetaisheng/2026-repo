...
> 更新日期: 2026-01-31
> 新增内容: 社交媒体人气数据、国家/地区效应、Partner独立分析模块
...
# 人气分组（处理0值和缺失）
popularity_log = log(popularity + 1)  # 对数变换
popularity_tier = cut(popularity, [0, 5, 20, 50, max])  # 分层
```
...
Y_i = \beta_0 + \underbrace{\beta_1 \cdot \text{Age}_i + \beta_2 \cdot \text{Age}_i^2}_{\text{年龄效应（含非线性）}} 
    + \underbrace{\sum_k \gamma_k \cdot \mathbf{1}[\text{Industry}_i = k]}_{\text{职业类别效应}} 
    + \underbrace{\delta \cdot \mathbf{1}[\text{US}_i]}_{\text{国家效应}} 
    + \underbrace{\phi \cdot \log(\text{Popularity}_i + 1)}_{\text{人气效应}} 
    + \epsilon_i
$$
...
# 1.4 人气数据处理
pop_df['celeb_pop_log'] = np.log(pop_df['Celebrity_Average_Popularity_Score'] + 1)
pop_df['partner_pop_log'] = np.log(pop_df['ballroom_partner_Average_Popularity_Score'] + 1)
```
...
analysis_df = contestant_df.merge(
    ...
    pop_df[['celebrity_name', 'season', 'celeb_pop_log']],
    on=['celebrity_name', 'season']
)
...
partner_analysis_df = analysis_df.merge(
    ...
    pop_df[['celebrity_name', 'season', 'partner_pop_log']],
    on=['celebrity_name', 'season']
)
...
# 标准化连续变量
for col in ['celebrity_age_during_season', 'celeb_pop_log']:
...
sns.scatterplot(x='celeb_pop_log', y='avg_fan_vote_share', 
                hue='celebrity_industry', data=analysis_df, ax=ax)
ax.set_xlabel('Log(Popularity + 1)')
...
**检验方法:**
1. 相关性分析: `corr(popularity_log, avg_fan_vote_share)`
